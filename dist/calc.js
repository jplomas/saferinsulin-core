...
class Calc {
    ...
    ongoingRate(passedCurrent, passedPrevious, passedRate) {
    ...
    createGovernance(obj) {
        ...
        if (obj.f === 'b' || obj.f === 'c') {
            const h = `${this.rateToHex(obj.rate) + this.glucoseToHex(obj.current) + this.glucoseToHex(obj.previous)}-${obj.f}${this.getHexDate(n)}`;
            return h;
        }
        return false;
    }
    ongoingRate100(passedCurrent, passedPrevious, passedRate) {
        const r = {
            rateNum: 0,
            rate: '',
            advice: { text: [''], type: '' },
        ...
        if (Number.isNaN(parseFloat(passedCurrent)) ||
            Number.isNaN(parseFloat(passedPrevious)) ||
            Number.isNaN(parseFloat(passedRate))) {
            return false;
        }
        const current = parseFloat(passedCurrent);
        const previous = parseFloat(passedPrevious);
        const rate = parseFloat(passedRate);
        let A11 = 'LOGIC ERROR';
        if (current <= 2.2) {
            A11 = '1';
        }
    ...
    ongoingRate200(passedCurrent, passedPrevious, passedRate) {
        const r = {
            rateNum: 0,
            rate: '',
            advice: { text: [''], type: '' },
